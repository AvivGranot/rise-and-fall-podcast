  ...
  return (
    <div className="min-h-screen bg-[#1a1a1a]">
      ...
      <section className="relative h-[60vh] flex items-end">
        ...
        <div className={`relative z-10 px-6 md:px-12 lg:px-24 pb-12 ${isRTL ? 'text-right w-full' : ''}`}>
          <p className="text-sm uppercase tracking-[0.3em] text-white/70 mb-4">{t('nav.contact')}</p>
        ...
        <div className="max-w-6xl mx-auto">
          <div className={`grid md:grid-cols-2 gap-16 ${isRTL ? 'direction-rtl' : ''}`}>
            ...
            <div className={isRTL ? 'text-right' : ''}>
              <h2 className="text-2xl font-light mb-8">{t('contact.sendMessage')}</h2>
              <ContactForm />
            </div>
            ...
            <div className={isRTL ? 'text-right' : ''}>
              <h2 className="text-2xl font-light mb-8">
                {isRTL ? 'התחברו איתנו' : 'Connect With Us'}
              </h2>
              ...
              {/* Listen On */}
              <div className="mb-12">
                <h3 className="text-sm uppercase tracking-wider text-gray-500 mb-4">{t('contact.listenOn')}</h3>
                <div className={`flex flex-wrap gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <a
                    ...
                    className={`flex items-center gap-2 px-4 py-2 border border-[#333] hover:border-[#1DB954] hover:bg-[#1DB954] hover:text-black transition-all text-sm uppercase tracking-wider ${isRTL ? 'flex-row-reverse' : ''}`}
                  >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`flex items-center gap-2 px-4 py-2 border border-[#333] hover:border-[#9933FF] hover:bg-[#9933FF] hover:text-white transition-all text-sm uppercase tracking-wider ${isRTL ? 'flex-row-reverse' : ''}`}
                  >
                    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`flex items-center gap-2 px-4 py-2 border border-[#333] hover:border-[#FF0000] hover:bg-[#FF0000] hover:text-white transition-all text-sm uppercase tracking-wider ${isRTL ? 'flex-row-reverse' : ''}`}
                  >
                    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
              ...
              <div className="p-8 bg-[#111] rounded">
                <h3 className="text-xl font-light mb-4">
                  {isRTL ? 'אל תפספסו פרק' : 'Never Miss an Episode'}
                </h3>
                <p className="text-gray-400 mb-6">
                  {isRTL
                    ? 'הצטרפו לניוזלטר שלנו לתוכן בלעדי, עדכונים מאחורי הקלעים, וגישה מוקדמת לפרקים חדשים.'
                    : 'Join our newsletter for exclusive content, behind-the-scenes updates, and early access to new episodes.'}
                </p>
                <form className={`flex gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <input
                    type="email"
                    placeholder={isRTL ? 'האימייל שלך' : 'Your email'}
                    className={`flex-1 px-4 py-3 bg-transparent border border-[#333] focus:border-white focus:outline-none transition-colors ${isRTL ? 'text-right' : ''}`}
                  />
